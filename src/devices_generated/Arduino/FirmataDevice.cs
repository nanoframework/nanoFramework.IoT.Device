...
namespace Iot.Device.Arduino
{
    ...
    internal sealed class FirmataDevice : IDisposable
    {
                    ...
                    switch (sysCommand)
                    {
                        case FirmataSysexCommand.REPORT_FIRMWARE:
                            ...
                            // Byte 0 is the command (0x79) and can be skipped here, as we've already interpreted it
                            {
                                ...
                                SpanByte bytesReceived = new byte[stringLength];
        ...
        private string PrintfFromByteStream(string fmt, in SpanByte bytesReceived, int startOfArguments)
        {
            string output = fmt;
