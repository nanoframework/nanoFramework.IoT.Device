...
namespace GoPiGo3.Samples
{
    public partial class Program
    {
        private static void TestUltrasound()
        {
            ...
            Debug.WriteLine($"Test {ultraSonic.SensorName} on port {ultraSonic.Port}. Gives the distance. Press enter to stop the test.");
            AddLines();
        ...
        private static void TestBuzzer()
        {
            ...
            Debug.WriteLine($"Play notes on {buzzer.SensorName} on port {buzzer.Port} in loops, changing duty cycle 50 (default), 80 and 10. Press a enter to stop the test.");
            buzzer.Start();
            Debug.WriteLine($"Playing with duty cycle = {buzzer.Duty}");
            int i = 0;
            while (!Console.KeyAvailable)
            {
        ...
        private static void TestPotentiometer()
        {
            ...
            Debug.WriteLine($"Control the {buzzer.SensorName} on port {buzzer.Port} with the {potentiometerSensor.SensorName} on port {buzzer.Port}. Press enter to stop the test.");
            AddLines();
        ...
        private static void TestSound()
        {
            ...
            Debug.WriteLine($"Test {soundSensor.SensorName} on port {soundSensor.Port}. Press a key to finish the test");
            AddLines();
            while (!Console.KeyAvailable)
            {
                Console.CursorLeft = 0;
        ...
        private static void TestRelay()
        {
            ...
            Debug.WriteLine($"Test {relay.SensorName} on port {relay.Port}, will change the state of the relay from on to off, reverse the polarity, turn on and off, reverse the polarity and turn on an off again.");
            Debug.WriteLine($"{relay.SensorName}, Value: {relay.Value}, state: {relay} IsInverted: {relay.IsInverted}");
        ...
        private static void TestButton()
        {
            ...
            Debug.WriteLine($"Test {button.SensorName} on port {button.Port}. Press enter to stop the test.");
            AddLines();
            while (!Console.KeyAvailable)
            {
                Console.CursorLeft = 0;
        ...
        private static void TestLedPwmLightSensor()
        {
            ...
            Debug.WriteLine($"Test {lightSensor.SensorName} on port {lightSensor.Port} controlling a {ledPwm.SensorName} on port {ledPwm.Port}. The intensity of the led will change proportionnaly to the intensity read by the sensor. Press enter to stop the test");
            AddLines();
            while (!Console.KeyAvailable)
            {
                Console.CursorLeft = 0;
