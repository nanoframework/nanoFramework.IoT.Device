...
{
    /// <summary>
    /// List class for type Sensor that has been automatically generated
    /// </summary>
    public class ListSensor : IEnumerable
    {
        ...
        public ListSensor(IEnumerable collection)
        {
            if (collection == null)
            {
        ...
        public ListSensor(int capacity)
        {
            if (capacity < 0)
            {
        ...
        /// The collection itself cannot be null, but it can contain elements that are null, if type Sensor is a reference type.
        /// </param>
        /// <exception cref="System.ArgumentNullException">collection is null.</exception>
        ...
        {
            private int _index;
            private ListSensor _collection;

            /// <summary>
            /// Create an enumerator of the collection
            /// </summary>
            /// <param name="collection"></param>
            public Enumerator(ListSensor collection)
            {
                _index = 0;
            ...
            /// </summary>
            public Sensor Current => _collection[_index];
        ...
        public bool Contains(Sensor item)
        {
            ...
            {
                if (((Sensor)elem).GetHashCode() == item.GetHashCode())
                {
                    return true;
                }
            }
        ...
        public void CopyTo(int index, Sensor[] array, int arrayIndex, int count)
        {
            ...
            {
                array[arrayIndex + i] = (Sensor)_list[i];
            }
        }
        ...
        public ListSensor GetRange(int index, int count)
        {
            if ((index < 0) || (count < 0))
            {
                throw new ArgumentOutOfRangeException();
            ...
            var list = new ListSensor();
            for (int i = index; i < count; i++)
            {
                list.Add((Sensor)_list[i]);
            }
        ...
        public int IndexOf(Sensor item, int index, int count) => _list.IndexOf(item, index, count);
        ...
        public int IndexOf(Sensor item, int index) => IndexOf(item, index, _list.Count - index);
        ...
        public int IndexOf(Sensor item) => IndexOf(item, 0, _list.Count);
        ...
        public void Insert(int index, Sensor item) => _list.Insert(index, item);

        /// <summary>
        /// Inserts the elements of a collection into the System.Collections.Generic.List at the specified index.
        /// </summary>
        /// <param name="index">The zero-based index at which the new elements should be inserted.</param>
        /// <param name="collection">The collection whose elements should be inserted into the System.Collections.Generic.List. The collection itself cannot be null, but it can contain elements that are null, if type Sensor is a reference type.</param>
        /// <exception cref="System.ArgumentNullException">collection is null.</exception>
        /// <exception cref="System.ArgumentOutOfRangeException">index is less than 0. -or- index is greater than System.Collections.Generic.List.Count.</exception>
        ...
        public int LastIndexOf(Sensor item) => LastIndexOf(item, _list.Count - 1, _list.Count);
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">index is outside the range of valid indexes for the System.Collections.Generic.List.</exception>
        public int LastIndexOf(Sensor item, int index) => LastIndexOf(item, index, _list.Count - index);
        ...
        public int LastIndexOf(Sensor item, int index, int count)
        {
            ...
            {
                if (((Sensor)_list[i]).GetHashCode() == item.GetHashCode())
                {
                    return i;
                }
            }
        ...
        {
            Sensor[] array = new Sensor[_list.Count];
            for (int i = 0; i < array.Length; i++)
            {
                array[i] = (Sensor)_list[i];
            }
