...
namespace Iot.Device.CharacterLcd.Samples
{
    internal class ExtendedSample
    {
        ...
        {
            using (Timer timer = new Timer(TimerCallbackClock, lcd, 0, 100))
            {
                // Adjust for something else
                Thread.Sleep(Timeout.Infinite);
            }
        }

        private static void TimerCallbackClock(object callback)
        {
            Hd44780 lcd = (Hd44780)callback;
            ...
            try
            {
                ...
                lcd.SetCursorPosition(0, 0);
                lcd.Write(String.Format("{0:dddd}", now));
                lcd.SetCursorPosition(0, 1);
                lcd.Write(String.Format("{0:M} {0:yyyy}", now, now));
                lcd.SetCursorPosition(0, 2);
                lcd.Write("It is now ");
                lcd.SetCursorPosition(0, 3);
                lcd.Write(String.Format("{0}", now.ToString()));
