...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        public void SetMotorPower(SensorPort port, int powerPercent)
        {
            ...
            {
                PortWrite($"port {(byte)port} ; pwm ; set {powerPercent / 100.0}\r");
            }
        }
        ...
        public void SetMotorLimits(SensorPort port, double powerLimit = 0.1)
        {
            ...
            PortWrite($"port {(byte)port} ; plimit {powerLimit}\r");
        }
        ...
        public void SetMotorBias(SensorPort port, double bias = 0.1)
        {
            ...
            PortWrite($"port {(byte)port} ; bias {bias}\r");
        }
        ...
        public void MoveMotorForSeconds(SensorPort port, double seconds, int speed, bool blocking = false, CancellationToken token = default)
        {
            ...
            // Set continuous reading
            SelectCombiModesAndRead(port, new int[] { 1, 2, 3 }, false);
            PortWrite($"port {(byte)port} ; pid {(byte)port} 0 0 s1 1 0 0.003 0.01 0 100; set pulse {speed} 0.0 {seconds} 0\r");
            if (blocking)
            {
                token.WaitHandle.WaitOne((int)(seconds * 1000), true);
            }
        }
        ...
        public void MoveMotorToAbsolutePosition(SensorPort port, int targetPosition, PositionWay way, int speed, bool blocking = false, CancellationToken token = default)
        {
            ...
            PortWrite($"port {(byte)port} ; pid {(byte)port} 0 1 s4 0.0027777778 0 5 0 .1 3 ; set ramp {actualPositionDouble} {newPosition} {duration} 0\r");
            if (blocking)
            {
                token.WaitHandle.WaitOne((int)(duration * 1000), true);
            }
        }
        ...
        public void MoveMotorToPosition(SensorPort port, int targetPosition, int speed, bool blocking = false, CancellationToken token = default)
        {
            ...
            if (blocking)
            {
                ...
                {
                    token.WaitHandle.WaitOne(5, true);
                    pos = motor.Position;
                }
            }
        }
        ...
        public void SelectModeAndRead(SensorPort port, int mode, bool readOnce)
        {
            ...
            if ((mode > sensor.ModeDetails.Length - 1) || (mode < 0))
            {
        ...
        public void SelectCombiModesAndRead(SensorPort port, int[] modes, bool readOnce)
        {
            ...
            {
                for (int i = 0; i < sensor.ModeDetails.Length; i++)
                // if (!sensor.ModeDetails.Select(m => m.Number == mode).Any())
                {
                    if (sensor.ModeDetails[i].Number == mode)
                    {
                        found = true;
                        break;
                    }
        ...
        public void WriteBytesToSensor(SensorPort port, SpanByte data, bool singleHeader)
        {
            ...
            {
                command.Append($"{data[i].ToString("X2")} ");
            }
        ...
        public void WaitForSensorToConnect(SensorPort port, CancellationToken token = default)
        {
            ...
            {
                token.WaitHandle.WaitOne(50, true);
            }
        ...
        private void CheckForFirmwareAndUpload()
        {
            ...
            if (prompt.Contains(BootloaderSignature))
            {
                ...
                // Load both files
                byte[] firmware = Resource.GetBytes(Resource.BinaryResources.firmware);
                byte[] signature = Resource.GetBytes(Resource.BinaryResources.signature);
                ...
                _port.Write("\x03\r");
                Thread.Sleep(10);
                Debug.WriteLine(PortReadExisting());
        ...
        retryVoltage:
            ...
            PortReadLine();
            var rawV = GetRawVoltage().Split(' ').CleanEmpltySpaces();
            try
            {
                _vin = ElectricPotential.FromVolts(double.Parse(rawV[0]));
            }
            catch (Exception ex)
            {
                inc++;
                                        ...
                                        case SensorType.SpikePrimeColorSensor:
                                            ...
                                            switch (mode)
                                            {
                                                case 5:
                                                    ...
                                                    {
                                                        color.Color = Color.FromArgb((byte)(Convert.ToInt32(elements[inc++]) * 255 / 1024),
        ...
        retrySignature:
            ...
            byte[] resultSignature = new byte[signature.Length];
            for (int i = 0; i < resultSignature.Length; i++)
            {
                resultSignature[i] = Convert.ToByte(signature[i], 16);
            }

            // Second result of the version is the date
            DateTime dt = version[1].ToDateTime();
