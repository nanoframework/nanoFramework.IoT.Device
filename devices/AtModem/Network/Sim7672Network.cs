...
namespace Iot.Device.AtModem.Network
{
    ...
    /// </summary>
    public class Sim7672Network : INetwork
    {
        ...
        internal Sim7672Network(ModemBase modem)
        {
            _modem = modem;
        ...
        /// <inheritdoc/>
        public bool Connect(PersonalIdentificationNumber pin = null, AccessPointConfiguration apn = null, int maxRetry = 15)
        {
            _apn = apn;
            _pin = pin;
        ...
        private bool ConnectInternal(int maxRetry = 15)
        {
            AtResponse response;
        ...
        Retry:
            ...
            WaitForSignal:
                ...
                response = _modem.Channel.SendCommand($"AT+CGAUTH=1,0,{(string.IsNullOrEmpty(_apn.UserName) ? "\"\"" : $"\"{_apn.UserName}\"")}{(string.IsNullOrEmpty(_apn.Password) ? ",\"\"" : $",\"{_apn.Password}\"")}");
                if (!response.Success)
                {
                    if (maxRetry-- > 0)
                    {
            ...
            // Make the connection
            response = _modem.Channel.SendCommand($"AT+CGACT=1,1");
            if (!response.Success)
            {
                if (maxRetry-- > 0)
                {
        ...
        {
            var response = _modem.Channel.SendCommandReadSingleLine("AT+CGPADDR=1", "+CGPADDR");
            if (response.Success)
            {
        ...
        {
            var response = _modem.Channel.SendCommand("AT+CGACT=0,1");
        ...
        public void Dispose()
        {
            ...
            AutoReconnect = false;
            Disconnect();
            _modem.Channel.SendCommand("AT+CRESET");
        }
    }
}
