...
{
    [TestClass]
    public class At24c256Tests
    {
        static I2cDevice _i2cDevice;
        static At24c256 _eeprom;
        static Random _random;

        // Expected device metrics for At24c256 from datasheet (page 16 section 6. Memory Organization).
        // https://ww1.microchip.com/downloads/aemDocuments/documents/OTH/ProductDocuments/DataSheets/At24c256C-AT24C256C-Data-Sheet-DS20006270B.pdf
        const int _expectedSize = 32768;
        const int _expectedPageSize = 64;
        const int _expectedPageCount = 512;
        ...
        public void SetupTests()
        {
            try
            {
                ...
                // Setup device. 
                I2cConnectionSettings i2cSettings = new I2cConnectionSettings(1, At24c256.DefaultI2cAddress);
                _i2cDevice = new I2cDevice(i2cSettings);
                _eeprom = new At24c256(_i2cDevice);
        ...
        {
            Assert.ThrowsException(typeof(ArgumentNullException), () => new At24c256(null));
        }
        ...
        public void Write_Writes_To_Device_Correctly_Within_Addressable_Range_Of_Device()
        {
            ...
            {
                Assert.AreEqual(randomData[k], dataRead[k], $"{k}");
            }
        }
