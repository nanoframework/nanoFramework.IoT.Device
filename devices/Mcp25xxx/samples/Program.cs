...
// Commenting this one as no Enum.GetValues
// In this case, the enum is large and have a lot of different values
...
void ConsoleWriteRegisterItemDetails(IRegister register)
{
    ...
    {
        if (field.MemberType == System.Reflection.MemberTypes.Property)
        {
            Debug.WriteLine($"{field.Name,15}: {field.GetValue(register)}");
        }
    }
}
...
void ReadRxBufferInteral(Mcp25xxx mcp25xxx, RxBufferAddressPointer addressPointer, int byteCount)
{
    ...
    Debug.Write($"{addressPointer,10}: ");

    foreach (byte value in data)
    {
        Debug.Write($"0x{value:X2} ");
    }
