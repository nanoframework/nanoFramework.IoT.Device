...
namespace nanoFramework.HomeAssistant
{
    ...
    public sealed class HomeAssistantClient
    {
        ...
        /// <param name="mqttClientIdPrefix">Prefix for the MQTT client ID.
        /// The full ID is this prefix followed by the sanitized <see cref="HomeAssistantDeviceInfo.Id"/>, so it identifies the device and stays the same across reconnections and reboots.
        /// This matters whenever a last-will-testament is in use: a client ID that changed per connection would leave the broker holding a stale session whose will fires after the new session has already published its "online" availability, marking the device offline while it is connected.
        /// It also means device IDs must be unique after sanitization, not merely distinct as written: <see cref="HomeAssistantDeviceInfo.Id"/> is lowercased; ASCII whitespace, '-' and '_' are all mapped to a single separator; runs of separators are collapsed; and leading/trailing separators are dropped. For example, 'Gate Sensor', 'gate_sensor' and '--GATE--SENSOR--' all resolve to the same identity.
        /// Two devices that collide this way share both the topic root and the client ID, so they will publish over each other's state and repeatedly disconnect each other from the broker.</param>
