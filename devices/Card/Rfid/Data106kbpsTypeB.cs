...
namespace Iot.Device.Rfid
{
    ...
    public class Data106kbpsTypeB
    {
        ...
        public Data106kbpsTypeB(byte[] atqb)
        {
            Command = atqb[0];
            NfcId = new byte[4];
            // Original: atqb.AsSpan().Slice(1, 4).CopyTo(NfcId);
            Array.Copy(atqb, 1, NfcId, 0, 4);
            ApplicationData = new byte[4];
            // Original: atqb.AsSpan().Slice(5, 4).CopyTo(ApplicationData);
            Array.Copy(atqb, 5, ApplicationData, 0, 4);
            BitRates = atqb[9];
